...
public class ImageViewActivity extends FragmentActivity {
    ...
    private IALatLng touchedLatLng;
    private float error;
    ...
    private IALocationListener mLocationListener = new IALocationListenerSupport() {
        ...
        public void onLocationChanged(IALocation location) {
            ...
            if (mImageView != null && mImageView.isReady()) {
                ...
                if (mImageView.getTouchStatus()!=false) {
                    Location locationA = new Location("point A");
                    locationA.setLatitude(location.getLatitude());
                    locationA.setLongitude(location.getLongitude());

                    Location locationB = new Location("point B");
                    locationB.setLatitude(touchedLatLng.latitude);
                    locationB.setLongitude(touchedLatLng.longitude);

                    error = locationA.distanceTo(locationB);

                    infoText.setText(String.format("error = %f m",error));
                }
        ...
        mImageView.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if(event.getAction() == MotionEvent.ACTION_UP){
                    ...
                    mImageView.setTouchedCoordinate(event.getX(),event.getY());
                    mImageView.invokeFirstTouch();
                    PointF touchedPoint = new PointF(event.getX(),event.getY());
                    touchedLatLng = mFloorPlan.pointToCoordinate(touchedPoint);
                    Log.i(TAG,"lat:"+touchedLatLng.latitude+"lon:"+touchedLatLng.longitude);
                    //Log.i(TAG,"error:"+error+" m");
                    //infoText.setText(String.format("error = %f m",error));
                    return true;
                }
                return false;
